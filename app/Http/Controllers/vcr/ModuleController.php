...
use App\Models\vcr\Module;
use App\Library\ZipHelpers;
use App\Library\vcrHelpers;
...
class ModuleController extends Controller {
	...
	public function index( Request $request)
	{
		...
		return view('vcr.module.index',$this->data);
	}
    ...
    function getCreate()
    {
        ...
        return view('vcr.module.create',$this->data);       
    ...
    function postCreate( Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            if($request->input('creation') == 'manual')
            {
                if($where =="")                
                {
                    return Redirect::to('vcr/module/create')                    
            ...
            if(count($row) >= 1)
            {
                ...
                return Redirect::to('vcr/module/rebuild/'.$id->module_id);        
            } else {
                return Redirect::to('vcr/module');
            }
    ...
    function getDestroy( $id = null )
    {
        $row = \DB::table('tb_module')->where('module_id', $id)
                                ->get();        
        if(count($row) <= 0){
            return Redirect::to('vcr/module')->with('messagetext','Can not find module')->with('msgstatus','error');   
    ...
    function getConfig( $id )
    {
		...
		return view('vcr.module.config',$this->data);        
    ...
    function postSaveconfig( Request $request) 
    {
        ...
        if ($validator->passes()) {
            ...
            return Redirect::to('vcr/module/config/'.$request->input('module_name'))
    ...
    function getSql(Request $request , $id )
    {
		...
		return view('vcr.module.sql',$this->data);
    ...
    function postSavesql( Request $request ,$id )
    {
        ...
        try {   
            ...
            // Do something when query fails. 
            $error ='Error : '.$select .' '.$where.' '.$group ;
            return Redirect::to('vcr/module/sql/'.$request->input('module_name'))
       ...
       \DB::table('tb_module')
        ...
        return Redirect::to('vcr/module/sql/'.$row->module_name)
    ...
    function getTable( $id )
    {
		...
		$this->data['module_name'] = $row->module_name;
         $this->data['type']     = $row->module_type;
		return view('vcr.module.table',$this->data);
    ...
    function getForm( $id )
    {
		...
		$this->data['module_name'] = $row->module_name;
         $this->data['type']     = $row->module_type;
		return view('vcr.module.form',$this->data);
    ...
    public function postSaveform( Request $request)
    {
        ...
        return Redirect::to('vcr/module/form/'.$row->module_name)
    ...
    public function getEditform(Request $request, $id )
    {
        ...
        $this->data['module'] = 'module';
        $this->data['module_name'] = $row->module_name;
        return view('vcr.module.field',$this->data);
    } 
   ...
   function postSaveformfield( Request $request)
    ...
    public function postSavetable( Request $request)
    {
        ...
        return Redirect::to('vcr/module/table/'.$row->module_name)
   ...
   function getPermission( $id )
    {
            ...
            */   
            ...
            return view('vcr.module.permission',$this->data);
    ...
    public function postSavepermission( Request $request)
    {
        ...
        */         
        ...
        return Redirect::to('vcr/module/permission/'.$row->module_name)
    ...
    function getBuild( $id )
    {
        ...
        return view('vcr.module.build',$this->data);
    ...
    function getFormdesign( Request $request , $id)
    {
        ...
        $this->data['module_name'] = $row->module_name;
        $this->data['type']           = $row->module_type;
        return view('vcr.module.formdesign',$this->data);    
    }   
    ...
    public function postFormdesign( Request $request)
    {
      ...
      \DB::table('tb_module')
        ...
        return Redirect::to('vcr/module/formdesign/'.$row->module_name)
    ...
    function getSub( Request $request, $id ='')
    {
		...
		return view('vcr.module.sub',$this->data);    
    ...
    function postSavesub( Request $request)
    {
        ...
        if ($validator->passes()) {                
            ...
            return Redirect::to('vcr/module/sub/'.$row->module_name)
            ->with('messagetext','Master Has beed added Successful.')->with('msgstatus','success');  

        }    else {

            return Redirect::to('vcr/module/sub/'.$request->get('module_name'))
    ...
    function getConn( Request $request , $id )
    {
        ...
        return view('vcr.module.connection',$this->data);
    }   
   ...
   function configGrid ( $field , $alias , $type, $sort ) {
    ...
    function postDobuild( Request $request ,$id )
    {
        ...
        $url = 'vcr/module/rebuild/'.$id."?rebuild=y&c={$c}&m={$m}&g={$g}&f={$f}&v={$v}&fg={$fg}&fv={$fv}&ff={$ff}";  
    ...
    public function getRebuild(Request $request, $id = 0)
    {
            ...
            if($row->module_type =='generic')
            {
                ...
                $template = base_path().'/resources/views/vcr/module/template/blank/';
            ...
            {
                $template   = base_path().'/resources/views/vcr/module/template/report/';
                $controller = file_get_contents( $template.'controller.tpl' );
                $grid       = file_get_contents(  $template.'grid.tpl' );
                $model       = file_get_contents(  $template.'model.tpl' );  
            ...
            if($row->module_type =='addon')
            {
                ...
                $template = base_path().'/resources/views/vcr/module/template/native/';
                $controller = file_get_contents(  $template.'controller.tpl' );
                $grid = file_get_contents(  $template.'grid.tpl' );               
                $view = file_get_contents(  $template.'view.tpl' );
                $form = file_get_contents(  $template.'form.tpl' );
        ...
        if($request->ajax() == true && \Auth::check() == true)
        {
            ...
            return Redirect::to('vcr/module')->with('messagetext','Proccess successfull')->with('msgstatus','success');   
        }
...
// End Routes for ".$row->module_name." 
    ...
    function postPackage( Request $request) 
    {
        ...
        return view('vcr.module.package',$this->data);  
  ...
  function postInstall( Request $request ,$id =0)
  {
    ...
    if ($validator->passes()) {
      ...
      $data = \vcrHelpers::cf_unpackage($path);
      ...
      self::createRouters();
          ...
          return Redirect::to('vcr/module')->with('messagetext','Module Installed' . $msg)->with('msgstatus','success');
    }  else  {
         return Redirect::to('vcr/module')->with('messagetext','Please select file to upload !')->with('msgstatus','error');
    } 
  ...
  function getSubform( Request $request ,$id =0)
  {
      ...
      //  print_r($this->data['subform']);
        ...
        return view('vcr.module.subform',$this->data);  
  }
   ...
   function postSavesubform( Request $request)
    {
        ...
        if ($validator->passes()) {                
            ...
            $row = \DB::table('tb_module')->where('module_id', $id)
                                    ->get();
            if(count($row) <= 0){
                return Redirect::to('vcr/module/subform/'.$request->get('module_name'))
                    ->with('messagetext', 'Can not find module')->with('msgstatus','error');      
            }
            $row = $row[0];                                    
            $this->data['row'] = $row;    
            ...
            */
            ...
            return Redirect::to('vcr/module/subform/'.$row->module_name)
  ...
  function getSubformremove( Request $request ,$id =0)
  {
    ...
    function getSource( Request $request ,$id)
    {
        ...
        return view('vcr.module.source',$this->data);
...
//       echo '<pre>'; print_r($config); echo '</pre>'; 
        ...
        return view('vcr.module.duplicate',$this->data); 
        ...
        if ($validator->passes()) {
            ...
            if(count($res)>=1)
            {
               ...
               // echo $row->module_id ; exit;
                ...
                return Redirect::to('vcr/module/rebuild/'.$row->module_id.'?mode=duplicate');
         
            } else {
                return Redirect::to('vcr/module')->with('messagetext','Failed to Duplicate Module !' )->with('msgstatus','error');
